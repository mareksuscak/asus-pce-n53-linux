...
#define RF_M_READ8(_pAd, _RegId, _pVal1, _pVal2, _BitMask)		\
{	\
	UCHAR RfReg = 0;									\
	RT30xxReadRFRegister(_pAd, _RegId, &RfReg);		\
	if (_pVal1 != NULL)								\
		*((PUCHAR)_pVal1) = RfReg & _BitMask;			\
	if (_pVal2 != NULL)								\
		*((PUCHAR)_pVal2) = RfReg & (~_BitMask);		\
}

#define RF_M_WRITE8(_pAd, _RegId, _Val, _BitMask)	\
{	\
	UCHAR RfReg = 0;											\
	RF_M_READ8(_pAd, _RegId, NULL, &RfReg, _BitMask);		\
	RfReg = (RfReg & ~_BitMask) | ((_Val) & _BitMask);			\
	RT30xxWriteRFRegister(_pAd, _RegId, RfReg);				\
}
