...
VOID ToneRadarProgram_v1(PRTMP_ADAPTER pAd, ULONG threshold)
{
	...
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R184, 0xf0);
	bbp = pAd->CommonCfg.CarrierDetect.delta << 4;
	bbp |= (pAd->CommonCfg.CarrierDetect.div_flag & 0x1) << 3;
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R185, bbp);

	/* program threshold*/
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R184, 0x34);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R185, (threshold & 0xff000000) >> 24);

	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R184, 0x24);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R185, (threshold & 0xff0000) >> 16);

	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R184, 0x14);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R185, (threshold & 0xff00) >> 8);

	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R184, 0x04);
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R185, threshold & 0xff);

	/* ToneRadarEnable v1 */
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R184, 0x05);
}
